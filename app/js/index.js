/*RENDERER PROCESS UI FUNCTIONS & COMMUNICATION W/SERVER*/
var {ipcRenderer, remote} = require('electron');  
var EventListener =  require('../js/system/eventListeners/clientIndexListener');
...
$(document).ready(function(){
  ...
  $(document).on('click', ".main-menu-option", function(){
    ...
    $(".main-menu-option").each(function(){
      if ($(this) != $target)
        $(this).parent().find('.sub-menu-container').each(function(){ $(this).slideUp() });
    });
    //show selected sub menus
    $target.parent().find('.sub-menu-container').each(function(){ $(this).slideToggle() });
  });
});
